...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateStorage() { const val = Array.from(osPrefRadios).find(r => r.checked)?.value; storageDetails.hidden = (val !== 'Tak'); }
  ...
  function updateMonitor() { const val = Array.from(monitorRadios).find(r => r.checked)?.value; monitorDetails.hidden = (val !== 'Tak'); }
  ...
  async function handleSubmit(event) {
    event.preventDefault(); status.textContent = ''; submitBtn.disabled = true;
    ...
    if (!email || !/^\S+@\S+\.\S+$/.test(email)) { status.textContent = 'Podaj poprawny adres e‑mail (pole jest wymagane).'; submitBtn.disabled = false; return; }
    const payload = {}; for (const [key, value] of formData.entries()) payload[key] = value;
    try {
      const resp = await fetch(form.action, { method: form.method, headers: { 'Accept': 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      if (resp.ok) { form.hidden = true; if (thanks) thanks.hidden = false; }
      else { const data = await resp.json().catch(() => ({})); status.textContent = (data && data.error) ? `Błąd: ${data.error}` : 'Wystąpił problem z wysyłką.'; }
    } catch (e) { status.textContent = 'Brak połączenia lub błąd po stronie usługi.'; }
